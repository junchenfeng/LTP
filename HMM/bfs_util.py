...
def update_state_parmeters(X_mat, Mx, 
	...
	Ti = len(O)
	llk_vec = get_llk_all_states(X_mat, O, E, H, J, item_ids,
								hazard_matrix, observ_prob_matrix, state_init_dist, state_transit_matrix, effort_prob_matrix, 
...
if __name__ == '__main__':
	...
	llk_vec_null = get_llk_all_states(X_mat, O, E, H, J, item_ids, hazard_matrix, observ_prob_matrix, state_init_dist, state_transit_matrix, effort_prob_matrix,False, False,'X')
	llk_vec_X = get_llk_all_states(X_mat, O, E, H, J, item_ids, hazard_matrix, observ_prob_matrix, state_init_dist, state_transit_matrix, effort_prob_matrix, False, True, hazard_state='X')
	llk_vec_Y = get_llk_all_states(X_mat, O, E, H, J, item_ids, hazard_matrix, observ_prob_matrix, state_init_dist, state_transit_matrix, effort_prob_matrix, False, True, hazard_state='Y')
	
	l_null = get_joint_state_llk(X_mat, llk_vec_null, 1, 0, 1)/get_single_state_llk(X_mat, llk_vec_null, 0, 0)
	l_x = get_joint_state_llk(X_mat, llk_vec_X, 1, 0, 1)/get_single_state_llk(X_mat, llk_vec_X, 0, 0)
	l_y = get_joint_state_llk(X_mat, llk_vec_Y, 1, 0, 1)/get_single_state_llk(X_mat, llk_vec_Y, 0, 0)
