...
def draw_multilevel_pi(pi_params, num_mixture, Mx):
	...
	max_pi_iter = 100	
	pi_iter = 0
	while not check_multi_level_pi(state_init_dist, num_mixture) and pi_iter<max_pi_iter:
...
def draw_multilevel_l(param_slow, param_fast, param_high, Mx):
	...
	if l_iter == l_max_iter:
		raise Exception('learning rates are not drew for iten %d.' % j)
	else:
...
def get_final_chain(param_chain_vec, start, end, is_exit, is_effort):
	...
	param_chain['mixture'] = np.vstack([param_chain_vec[i]['mixture'][select_idx, :] for i in range(num_chain)])
