...
class ARS:
    ...
    def insert(self, xnew=[], hnew=[], hprimenew=[]):
        ...
        self.z[1:-1] = (np.diff(self.h) - np.diff(self.x * self.hprime)) / -np.diff(
            self.hprime
        )
    ...
    def sampleUpper(self):
        ...
        xt = (
            ...
            + (
                -self.h[i]
                + np.log(self.hprime[i] * (self.cu * u - self.s[i]) + np.exp(self.u[i]))
            )
            / self.hprime[i]
        )
        ...
        if fprima(guess_low) * fprima(guess_high) < 0:
            try:
                ars = ARS(
                    f,
                    fprima,
                    xi=[guess_low, (guess_low + guess_high) / 2, guess_high],
                    lb=0.01,
                    ub=bnd,
                )
            except:
        ...
        for obs_key in self.state_dict.keys():
            ...
            if X[k] != 0:
                bnds.append(
                    (
                        -np.log(self.Lambda)
                        - (np.dot(X, self.betas) - X[k] * self.betas[k])
                    )
                    / X[k]
                )
        bnd = min(bnds)
