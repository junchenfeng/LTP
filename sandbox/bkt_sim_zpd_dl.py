...
state_transit_matrix = np.array([[[1,0,0],[0, 1-l0, l0],[0, 0, 1]],[[1,0,0],[0, 1-l1, l1],[0, 0, 1]]])
...
for i in range(N):
	...
	for t in range(T):
		if t ==0:
			...
			S = int( np.random.choice(3, 1, p=state_transit_matrix[y, S, :]) )
...
with open(proj_dir + '/data/bkt/test/single_sim_zpd_dl_x_1.txt','w') as f:
	for log in data:
