...
print(mcmc_s[0], mcm_s_g[0], mcmc_pi[0], mcmc_l[0])


# For external use
with open(proj_dir+'/data/bkt/full_point_estimation.txt', 'w') as f1:
	f1.write('em,%f,%f,%f,%f\n' %(em_s, em_g, em_pi, em_l))
	f1.write('mcmc_s,%f,%f,%f,%f\n' %(mcmc_s[0], mcm_s_g[0], mcmc_pi, mcmc_l[0]))


np.savetxt(proj_dir+'/data/bkt/full_mcmc_survival_parameter_chain.txt', mcmc_instance.parameter_chain, delimiter=',')
...
print(sHat, gHat, piHat, lHat)
print(em_s, em_g, em_pi, em_l)
print(mcmc_s, mcm_s_g, mcmc_pi, mcmc_l)
...
mcmc_auc, mcmc_r2 = get_auc(mcmc_s, mcm_s_g, mcmc_pi, mcmc_l, incomplete_data_array)
...
with open(proj_dir+'/data/bkt/incomplete_point_estimation.txt', 'w') as f4:
	...
	for x in mcmc_h0+mcmc_h1:
		f4.write(',%f' % 0.0)	
	f4.write('\n')
		

	f4.write('mcmc_s,%f,%f,%f,%f' %(mcmc_s[0], mcm_s_g[0], mcmc_pi, mcmc_l[0]))
	for x in mcmc_h0+mcmc_h1:
		f4.write(',%f' % x)
	f4.write('\n')
	
np.savetxt(proj_dir+'/data/bkt/incomplete_mcmc_survival_parameter_chain.txt', mcmc_instance.parameter_chain, delimiter=',')
