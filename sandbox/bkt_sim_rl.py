...
observ_matrix = np.array([[1-c[i][0],c[i][0]] for i in range(3)])
...
with open(proj_dir + '/data/bkt/test/single_sim_rl_x_1.txt','w') as f:
	for log in data:
...
for i in range(N):
	...
	for t in range(T):
		if t ==0:
			...
			if E ==1:
				# no pain no gain
				Sx = int( np.random.choice(3, 1, p=state_transit_matrix[S, :]) )
				if S==0:
					y_cnt += Sx
					n_cnt += 1-Sx
				S = Sx
...
print('correct rate')
print([y_cnt[i]/x_cnt[i] for i in range(3)]) #P(E_t=1,X_{t-1}=0)/P(X_{t-0})

print('invalid effort correct rate')
print([y0_cnt[i]/x0_cnt[i] for i in range(2)]) #P(E_t=1,X_{t-1}=0)/P(X_{t-0})

with open(proj_dir + '/data/bkt/test/single_sim_rl_x_1_e.txt','w') as f:
	for log in data:
